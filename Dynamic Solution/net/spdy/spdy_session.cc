...
void SpdySession::OnPing(spdy::SpdyPingId unique_id, bool is_ack) {
  ...
  DVLOG(0) << "SPDY RTT:" << ping_duration.InMicroseconds();
  ...
  // Fetching the environment variable value and processing as required:
    ...
    if(pings.size() < 10){
      ...
      DVLOG(0) << "NEW_SPDY_PING_MEDIAN_CALCULATED: "<<getenv("spdy_pings_median")<<std::endl;
      ...
      if(quic_median != nullptr){
        ...
        }else if(quic_val >= 1000000 && median_value >= 1000000){
    ...
    DVLOG(0) << "SPDY_PINGS_LIST: "<<getenv("spdy_pings")<<std::endl;
  //  -----------------------------------
